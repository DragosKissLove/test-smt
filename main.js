...
// Handle Roblox download
ipcMain.handle('download-roblox', async (event, versionHash) => {
  const functions = require('./functions');
  try {
    return await functions.downloadRobloxPlayer(versionHash, (message) => {
      mainWindow.webContents.send('roblox-progress', message);
    });
  } catch (error) {
...
ipcMain.handle('run-function', async (event, { name, args }) => {
  ...
  if (typeof functions[name] === 'function') {
    try {
      return await functions[name](...(Array.isArray(args) ? args : [args]));
    } catch (error) {
